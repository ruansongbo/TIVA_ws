...

{
    ...
    for (dmpnum=0;dmpnum<42;dmpnum++)
    {
        ...
        dmpdatas[dmpnum] = readI2C1(SlaveAddress, 0x74);

    }

    writeI2C1(SlaveAddress, DETECT_CTRL, 0xcc);//FIFO_RST | DMP_RST | DMP_EN | FIFO_EN
...
void loadfirmware(void)
{
    ...
    for (datanum=0;datanum<1929;datanum++)
    {
        ...
        if(lastbank!=bank)
        {
            writeI2C1(SlaveAddress, 0x6d, bank);
            writeI2C1(SlaveAddress, 0x6e, 0);
        }
        writeI2C1(SlaveAddress, 0x6f, dmpmemorydata[datanum]);
        lastbank=bank;
    }
}
...
void loadcfgupd(void)
{
    ...
    for (line=0;line<37;line++)
    {
        ...
        writeI2C1(SlaveAddress, 0x6d, bank);
        writeI2C1(SlaveAddress, 0x6e, offset);
        for (writingcounts=0;writingcounts<bytes2write;writingcounts++)
        {
            datacounts++;
            writeI2C1(SlaveAddress, 0x6f, dmpcfgupddata[datacounts]);
        }
        datacounts++;
    }
}
...
{
    writeI2C1(SlaveAddress, PWR_MGMT_1, 0x00);
    writeI2C1(SlaveAddress, SMPLRT_DIV, 0x04);
    writeI2C1(SlaveAddress, CONFIG, 0x06);
    writeI2C1(SlaveAddress, GYRO_CONFIG, 0x18);
    writeI2C1(SlaveAddress, ACCEL_CONFIG, 0x00);
    loadfirmware();
    loadcfgupd();
    writeI2C1(SlaveAddress, DMP_CFG_1, 0x03);
    writeI2C1(SlaveAddress, DMP_CFG_2, 0x00);
    writeI2C1(SlaveAddress, DETECT_CTRL, 0xcc);
}
